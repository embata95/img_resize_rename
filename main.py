...
print_name = input('If you want the new image names to printed enter 1, otherwise enter 0: ') == '1'
if not desired_format.startswith('.'):
...
for curr_file in os.listdir():
    if isfile(curr_file) and imghdr.what(curr_file) in ['jpeg', 'png']:
        ...
        if desired_format.lower() != '.png':
            image = image.convert('RGB')
        image.thumbnail((int(max_width), int(max_height)))
        if not os.path.exists(new_folder_name):
            os.mkdir(new_folder_name)
        final_name = img_name + desired_format
